...
class VideoProcessorClass(VideoProcessorBase):
    def __init__(self):
        ...
        # Use the "lite" model for real-time CPU inference.
        # "full"/"heavy" variants are noticeably slower and cause visible lag.
        ...
        # Real wall-clock timestamps for MediaPipe's VIDEO running mode.
        # Must be initialized ONCE here, not reset every frame.
    ...
    def _draw_squats_overlays(self, img, metrics):
        h, _ = img.shape[:2]
        cv2.putText(img, f"DEPTH: {metrics['depth_status']}", (20, h - 20),
    ...
    def _draw_curl_overlays(self, img, metrics):
        h, _ = img.shape[:2]
        cv2.putText(img, f"SWING: {metrics['swing_status']}", (20, h - 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    ...
    def _draw_lunge_overlays(self, img, metrics):
        h, _ = img.shape[:2]
        cv2.putText(img, f"BALANCE: {metrics['balance_status']}", (20, h - 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    ...
    def recv(self, frame):
        ...
        # image is BGR (from to_ndarray(format="bgr24")).
        # MediaPipe expects SRGB, so convert BGR -> RGB before wrapping it.
        mp_image = mp.Image(
            image_format=mp.ImageFormat.SRGB,
            data=cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        )

        # Real elapsed-time timestamp (monotonically increasing), required by
        # MediaPipe's VIDEO running mode for correct internal temporal smoothing.
        self._frame_timestamps_ms = int((time.time() - self._start_time) * 1000)
